...
namespace TISensorTagLibrary.CC2541
{
    ...
    public class Accelerometer : SensorBase
    {
        ...
        public async Task SetReadPeriod(byte time)
        {
            ...
            var charForUuid = await deviceService.GetCharacteristicsForUuidAsync(new Guid(SensorTagUuid.UUID_ACC_PERI));
    ...
    public class PressureSensor : SensorBase
    {
        ...
        {
            var charForUuid = await deviceService.GetCharacteristicsForUuidAsync(new Guid(SensorConfigUuid));
            if (charForUuid.Characteristics.Count > 0)
            {
                GattCharacteristic tempConfig = charForUuid.Characteristics[0];
                ...
                var calibrationCharacteristicList = await deviceService.GetCharacteristicsForUuidAsync(new Guid(SensorTagUuid.UUID_BAR_CALI));
                if (calibrationCharacteristicList != null && calibrationCharacteristicList.Characteristics.Count > 0)
                {
                    GattCharacteristic calibrationCharacteristic = calibrationCharacteristicList.Characteristics[0];
