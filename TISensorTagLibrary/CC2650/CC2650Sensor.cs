...
namespace TISensorTagLibrary.CC2650
{
    ...
    public class BatteryLevel : SensorBase
    {
        ...
        {
            var CharacteristicList = await deviceService.GetCharacteristicsForUuidAsync(new Guid(SensorDataUuid));
            BatteryLevelCharacteristic = null;
            if (CharacteristicList != null)
            {
                if (CharacteristicList.Characteristics.Count() > 0)
                {
                    BatteryLevelCharacteristic = CharacteristicList.Characteristics[0];
                    Debug.WriteLine("BatterySetup: List[0].Uuid=" + BatteryLevelCharacteristic.Uuid);
                }
                Debug.WriteLine("BatterySetup: List.Count=" + CharacteristicList.Characteristics.Count());
            }
            else
            {
...
#pragma warning restore
    ...
    public class IOService : SensorBase
    {
        ...
        public async override Task EnableSensor()
        {
            ...
            if (charForUuid.Characteristics.Count > 0)
            {
                configCharacteristic = charForUuid.Characteristics[0];
            ...
            {
                Debug.WriteLine("*IOService::EnableSensor: Error!");
            }
        }
#endif
    }
